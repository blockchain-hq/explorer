...
import { isAccountPda } from '@/app/utils/program-testing/pda';
import { derivePDA } from '@/app/utils/program-testing/solana';
...
export const useAutoDerivePDAs = (
    ...
    const [derivedPDAs, setDerivedPDAs] = useState<Map<string, DerivedPDA>>(new Map());
    ...
    const derivePDAsForInstruction = useCallback(async () => {
        ...
        const pdaAccounts = instruction.accounts.filter(account => isAccountPda(account as ModIdlAccount));

        for (const account of pdaAccounts as ModIdlAccount[]) {
            const canDerive = checkDependencies(account, formData, accountsAddressMap);
    ...
    }, [instruction, idl, formData, accountsAddressMap, connection, derivePDAsForInstruction]);
    ...
    for (const seed of seeds) {
        ...
        if (seed.kind === 'account') {
            if (seed.account) {
                ...
                const address = formData[accountName] || accountsAddressMap.get(accountName);
                if (!address) {
