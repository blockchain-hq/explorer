...
import { Program, AnchorProvider, AnchorError, Idl } from '@coral-xyz/anchor';
import { useConnection, useAnchorWallet } from '@solana/wallet-adapter-react';
import type { Keypair } from '@solana/web3.js';
import { PublicKey } from '@solana/web3.js';
import { toast } from 'sonner';
import { toAnchorType, derivePDA } from '@/app/utils/program-testing/solana';
import { toCamelCase } from '@/app/utils/program-testing/general';
import type { IdlInstruction, ModIdlAccount } from '@/app/types/idl-types';
import { useTransactionToast } from '@/app/hooks/useTransactionToast';
import { parseSolanaError, type ErrorWithLogs } from '@/app/utils/program-testing/error-parser';
    ...
    const execute = async (
        ...
        try {
            ...
            for (const [name, pubkey] of Object.entries(accountPubkeys)) {
                ...
                console.log(`${name}:`, pubkey.toBase58(), 'Balance:', balance / 1e9, 'SOL');
            }

            const signature = await program.methods[toCamelCase(instruction.name)](...anchorArgs)
            ...
            txToast.success(signature, `Instruction ${instruction.name} executed successfully`);
            ...
            const parsedError = parseSolanaError(error as Error | AnchorError | ErrorWithLogs);

            txToast.error(parsedError);

            const errorSignature = (error as unknown as { signature?: string })?.signature;
            if (errorSignature) {
                // addTransaction({
