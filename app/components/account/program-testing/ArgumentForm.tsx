        ...
        <div className="bg-card border border-border/50 rounded-md p-4 space-y-4">
            ...
            {args.map(arg => (
                <div key={arg.name} className="grid w-full items-center gap-3">
                    <Label htmlFor={arg.name} className="text-sm font-medium text-foreground flex items-center gap-2">
                        {arg.name}
                        {arg.type && <span className="text-xs text-muted-foreground/50">({arg.type.toString()})</span>}
                    </Label>
                    {validationErrors[arg.name] && <p className="text-red-500 text-sm">{validationErrors[arg.name]}</p>}
                    <Input
                        id={arg.name}
                        type="text"
