import { Copy, Check, ExternalLink, ChevronDown, ChevronUp, Maximize2 } from 'react-feather';
import { useState } from 'react';
import { Button } from '@/app/components/shared/ui/button';
import { formatLogs } from '@/app/utils/program-testing/error-parser';
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
export function TransactionToast({
    ...
    const truncatedSig = signature ? `${signature.slice(0, 8)}...${signature.slice(-8)}` : '';
            ...
            {logs && logs.length > 0 && (
                <div className="border-t pt-2 mt-1">
                    <div className={`flex items-center justify-between ${showLogs ? 'w-[90%]' : 'w-full'}`}>
                        ...
                        >
                            <span className="text-xs font-semibold">Transaction Logs ({logs.length})</span>
                            ...
                            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                                ...
                                <DialogContent className="max-w-3xl max-h-[80vh] bg-card border border-border/50 text-foreground">
                                    <DialogHeader className="border-b border-border/50">
                                        ...
                                        <DialogDescription>
                                            {signature && <span className="text-xs">Transaction: {truncatedSig}</span>}
                                        </DialogDescription>
                                    </DialogHeader>
