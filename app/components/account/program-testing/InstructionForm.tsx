...
import { useAutoDerivePDAs } from '@/app/hooks/useAutoDerivePDA';
...
const InstructionFormv2 = (props: InstructionFormv2Props) => {
    ...
    const { getInstructionState, updateInstructionState } = useProgramTesting();
    ...
    useEffect(() => {
        ...
        const hasData = Object.values(formData).some(value => value !== '' && value !== undefined && value !== null);
    ...
    useEffect(() => {
        ...
        const hasData = Array.from(accountsAddressMap.values()).some(value => value && value.trim() !== '');
    ...
    const derivedPDAs = useAutoDerivePDAs(instruction, idl, formData, accountsAddressMap);
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        const result = await execute(instruction, accountsAddressMap, formData, signersKeypairs);
