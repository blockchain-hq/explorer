...
export function parseSolanaError(error: Error | AnchorError | ErrorWithLogs): ParsedError {
    ...
    if (errorMessage.includes('program that does not exist')) {
        return {
            ...
            message: "The program you're trying to call doesn't exist on this cluster.",
            logs,
            suggestion:
    ...
    if (errorMessage.includes('insufficient funds') || logs.some(l => l.includes('insufficient funds'))) {
        return {
    ...
    if (errorMessage.includes('AccountNotFound') || errorMessage.includes('could not find account')) {
        return {
            ...
            message: "One or more accounts required for this instruction don't exist yet.",
            logs,
    ...
    if (errorMessage.includes('Invalid public key input') || errorMessage.includes('invalid public key')) {
        return {
            title: 'Invalid Address',
            message: 'One of the addresses provided is not a valid Solana public key.',
            logs,
            suggestion: 'Check that all addresses are correct and properly formatted.',
        };
    }
    ...
    if (errorMessage.includes('Failed to fetch') || errorMessage.includes('NetworkError')) {
        return {
            ...
            logs,
            suggestion: 'Check your internet connection and make sure the validator is running.',
        };
    }
    ...
    if (logs.length > 0) {
        ...
        if (logsText.includes('already in use')) {
            return {
                ...
                logs,
                suggestion: "Use a different account or check if it's already initialized.",
            };
        }
        ...
        if (logsText.includes('access violation') || logsText.includes('privilege')) {
            return {
