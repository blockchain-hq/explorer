...
import type { PDASeed } from '@/app/types/idl-types';
...
export const toBuffer = (value: unknown, type: IdlType) => {
    ...
    if (['u8', 'u16', 'u32', 'u64', 'i8', 'i16', 'i32', 'i64'].includes(type as string)) {
...
export const derivePDA = async (
    ...
    for (const seed of seeds) {
        ...
        if (seed.kind === 'arg' && seed.path) {
            ...
            const argType = idl.instructions.flatMap(ix => ix.args).find(arg => arg.name === seed.path)?.type;
    ...
    const derivationProgramId = pdaProgram ? new PublicKey(new Uint8Array(pdaProgram.value)) : new PublicKey(programId);
